...
pub enum ThresholdResponse {
    /// Declares that a fixed points of yes votes is needed to pass.
    /// It does not matter how many no votes are cast, or how many do not vote,
    /// as long as `points` yes votes are cast.
    ///
    /// This is the simplest format and usually suitable for small multisigs of trusted parties,
    /// like 3 of 5. (points: 3, total_points: 5)
    ///
    /// A proposal of this type can pass early as soon as the needed points of yes votes has been cast.
    AbsoluteCount { points: u64, total_points: u64 },

    /// Declares a percentage of the total points that must cast Yes votes, in order for
    /// a proposal to pass. The passing points is computed over the total points minus the points of the
    /// abstained votes.
    ///
    ...
    /// refer to the same group, each proposal will work with a different set of voter points
    /// (the ones snapshotted at proposal creation), and the passing points for each proposal
    ...
    ///
    /// Example: we set `percentage` to 51%. Proposal 1 starts when there is a `total_points` of 5.
    /// This will require 3 points of Yes votes in order to pass. Later, the Proposal 2 starts but the
    /// `total_points` of the group has increased to 9. That proposal will then automatically
